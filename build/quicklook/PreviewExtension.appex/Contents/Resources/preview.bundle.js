...
`;
  ...
  marked.use({
    ...
    renderer: {
      code({ text: text2, lang }) {
        if (lang === "mermaid") {
          return `<pre><code class="language-mermaid">${escapeHtml(text2)}</code></pre>`;
        }
        return false;
      }
    }
  });
  var purify2 = purify(window);
  function parseMarkdown(md) {
    const raw = marked.parse(md, { async: false });
    return purify2.sanitize(raw, {
      ADD_ATTR: ["id", "class"]
    });
  }
  function setBaseHref(dirUrl) {
  ...
  function rewriteRootRelativeAttrValue(value) {
    const t = value.trim();
    if (!t || t.startsWith("//")) return null;
    if (!t.startsWith("/")) return null;
    return `.${t}`;
  }
  function normalizeRootRelativeMediaUrls(root) {
    const pairs = [
  ...
  async function main() {
    ...
    try {
      ...
      setBaseHref(directoryFileUrl(path));
      const md = await file.text();
      root.innerHTML = parseMarkdown(md);
      normalizeRootRelativeMediaUrls(root);
    } catch (e) {
      root.textContent = e instanceof Error ? e.message : "Could not load this preview.";
    }
    await quicklook.finishedLoading();
  }
  void main();
